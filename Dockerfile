...
RUN apt-get update && apt-get -y upgrade  && apt-get install -y  \
    ...
    cd /tmp/nginx-1.22.1 && ./configure --prefix=/opt/nginx \
    --with-http_stub_status_module --with-http_ssl_module --with-http_mp4_module \
    --add-module=../nginx-rtmp-module-1.2.2 && make && make install && \
    ./configure --add-module=../nginx-http-flv-module-1.2.11 && make && make install && \
